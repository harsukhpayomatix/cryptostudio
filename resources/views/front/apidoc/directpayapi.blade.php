    ...
    "data": {
        ...
        "client": {
            ...
            "country": "GB"
                        ...
                        <pre><code class="language-text">{{ config('app.url') }}?responseCode=1&responseMessage=Test%20transaction%20processed%20successfully.&order_id=ODRDX3Y1683876328LBIJA1&customer_order_id=TX121212</code></pre>

                        <p>If the transaction fails, the user will be redirected to “response_url” with response
                            in query string as follows:</p>

                        <pre><code class="language-text">{{ config('app.url') }}?responseCode=0&responseMessage=Card%not%supported%for%testing.&order_id=ODRDX3Y1683876328LBIJA1&customer_order_id=TX121212</code></pre>
                        <pre><code class="language-json">{
    "responseCode": "7",
