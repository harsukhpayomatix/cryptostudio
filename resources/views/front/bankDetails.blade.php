...
<div class="row">
    <div class="col-lg-12">
        <form method="POST" action="{{ route('update.user.bank.details') }}" class="form-dark">
            @csrf
                                ...
                                <div class="col-lg-12 form-group">
                                    ...
                                    <div class="input-group mb-1">
                                        <select class="select2" name="wallet" id="wallet" style="width: 300px;">
                                            ...
                                            <option value="{{ $wallet->id }}" {{ $bank && $wallet->id == $bank->wallet?'selected':'' }}>{{ $wallet->name }}</option>
                                            @endforeach
                                        </select>
                                        <input type="text" class="form-control col-md-8" placeholder="Wallet ID" name="wallet_id"
                                        value="{{ $bank ? $bank->wallet_id : old('wallet_id') }}" />
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-lg-6 form-group">
                                    ...
                                    <textarea class="form-control" placeholder="Enter here.."
                                        name="address">{{ $bank ? $bank->address : old('address') }}</textarea>
                                </div>
                                <div class="col-lg-6 form-group">
                                    <label>Account Holder's Address </label>
                                    <textarea class="form-control" placeholder="Enter here.."
                                        name="account_holder_address">{{ $bank ? $bank->account_holder_address : old('account_holder_address') }}</textarea>
                                </div>
                                <div class="col-lg-12 form-group">
                                    <label>Additional Information </label>
                                    <textarea class="form-control" placeholder="Enter here.."
                                        name="additional_information">{{ $bank ? $bank->additional_information : old('additional_information') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
