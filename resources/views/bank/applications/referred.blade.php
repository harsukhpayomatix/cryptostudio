...
@section('content')
    ...
    <div class="row">
        <div class="col-md-6">
            <h4 class="mt-50">Declined Application List</h4>
        </div>
        <div class="col-md-6 text-right">
            <form action="{{ route('bank.applications.exportDeclinedApplications', request()->all()) }}" method="POST">
                @csrf
                ...
                <div class="card-header">
                    ...
                    <div>
                        ...
                        <div class="btn-group">
                            ...
                            <a href="{{ route('bank.applications.declined') }}" class="btn btn-danger btn-sm"
                                ...
                                @if (!empty($data) && $data->count())
                                    @foreach ($data as $key => $value)
                                        <tr>
                                            ...
                                            <td>
                                                @foreach (json_decode($value->extra_documents) as $key => $document)
                                                    File - {{ $key + 1 }} <a href="{{ getS3Url($document) }}"
                                                        target="_blank" class="btn btn-info btn-sm">Show</a><br>
                                                @endforeach
                                            </td>
                                            <td class="text-danger">{{ $value->referred_note }}</td>
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        <a class="dropdown-item approved" title="Approve Application"
                                                            data-id="{{ $value->id }}"
                                                            data-current_users_id="{{ auth()->guard('bankUser')->user()->id }}">
                                                            Approve
                                                        </a>
                                                        <a href="#declinedModel" data-toggle="modal"
                                                            class="dropdown-item" id="declined"
                                                            title="Declined Application" data-id="{{ $value->id }}"
                                                            data-current_users_id="{{ auth()->guard('bankUser')->user()->id }}">
                                                            Decline
                                                        </a>
    ...
    @include('bank.applications.partials.decline_model')
@endsection
