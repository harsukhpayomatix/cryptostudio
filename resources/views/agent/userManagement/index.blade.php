...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-md-6 text-right">
            <a class="btn btn-danger btn-sm" href="{{ route('user-management-agent-create') }}">Add New
                Merchant</a>
            <a href="{{ route('rp.user-management', ['type' => 'xlsx'] + request()->all()) }}"
                class="btn btn-primary btn-sm" id="ExcelLink"><i class="fa fa-download"></i> Export Excel </a>
        </div>
        <div class="col-lg-12">
            <div class="card mt-2">
                            ...
                            <tbody>
                                @foreach ($merchantManagementData as $k => $value)
                                    <tr>
                                        ...
                                        <td>{{ $value->agent_commission_master_card }} %</td>
                                        <td>
                                            @if ($value->email_verified_at != null)
                                                <span class="badge badge-success badge-sm">Email Verification -
                                                    Verified</span>
                                            @else
                                                <span class="badge badge-primary badge-sm">Email Verification -
                                                    Unverified</span>
                                            @endif
                                            <br>
                                            @if ($value->appStatus == '1')
                                                <span class="badge badge-primary badge-sm">Application - In Progress</span>
                                            @elseif($value->appStatus == '2')
                                                <span class="badge badge-primary badge-sm">Application - Incomplete</span>
                                            ...
                                            @if ($value->is_active == 1)
                                                <label class="custom-control overflow-checkbox"
                                                    ...
                                                    <input type="checkbox" class="form-check-input" name="is_active"
                                                        id="is_active{{ $value->id }}" data-id="{{ $value->id }}"
                                                        checked>
                                                    ...
                                                    @if ($value->appStatus == '')
                                                        <a href="{{ route('user-management-application-create', $value->id) }}"
                                                            class="dropdown-item"><i
                                                                class="fa fa-plus text-primary mr-2"></i> Create
                                                            Application</a>
                                                    @else
                                                        @if ($value->appStatus <= '2')
                                                            <a href="{{ route('user-management-application-edit', $value->id) }}"
                                                                class="dropdown-item"> <i
                                                                    class="fa fa-edit text-warning mr-2"></i> Edit
                                                                Application</a>
                                                        @endif
                                                        @if (!$value->is_whitelable)
