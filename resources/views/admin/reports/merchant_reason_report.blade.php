...
@section('title')
    Merchant Transaction Responses
@endsection

@section('breadcrumbTitle')
    <a href="{{ route('admin.dashboard') }}">Dashboard</a> / Merchant Transaction Responses
@endsection
@section('content')
                    ...
                    <div class="modal-body">
                        <div class="basic-form">
                            <div class="row ">
                                ...
                                {{-- <div class="form-group col-lg-6">
                                <label for="business_name">Select Reason</label>
                                <select name="reason" id="reason_name" data-size="7" data-live-search="true"
                                        class="select2 btn-primary fill_selectbtn_in own_selectbox" data-width="100%">
                                    <option selected value=""> -- Select Reason -- </option>
                                    @foreach ($all_reasons as $val)
                                        <option value="{{ $val }}"
                                            {{ (isset($_GET['reason']) && $_GET['reason'] == $key) ? '' : '' }}>
                                            {{ $val }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('reason'))
                                    <span class="help-block">
                                    <strong class="text-red">{{ $errors->first('reason') }}</strong>
                                </span>
                                @endif
                            </div> --}}
            ...
            <div class="card mt-1">
                <div class="card-header">
                    <div class="iq-header-title">
                        <h4 class="card-title">Merchant Transaction Responses <span class="text-red"> (
                    ...
                    <div>
                        <div class="btn-group">
                            ...
                            <a href="{{ route('merchant-transactions-reason-report') }}" class="btn btn-danger btn-sm"
                                style="border-radius: 0px 5px 5px 0px !important;">Reset</a>
                        </div>
                        @if (auth()->guard('admin')->user()->can(['export-merchant-transaction-responses']))
                            <a class="btn btn-primary btn-sm"
                                href="{{ route('merchant-transactions-reason-report-excle', request()->all()) }}"
                                data-filename="Merchant_Transaction_Reason_Report_Excel_" id="ExcelLink">
                    ...
                    <div class="table-responsive custom-table tableFixHead">
                        <table class="table table-striped table-borderless">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th style="width: 250px">Merchant</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($data as $key => $value)
                                    ...
                                    $rowspan_cnt = $value->count();
                                    ?>
                                <tr>
                                    @if ($i == 0)
                                    <td rowspan="{{ $rowspan_cnt }}" style="vertical-align: top;">{{ $loop->iteration }}</td>
                                    <td rowspan="{{ $rowspan_cnt }}" style="vertical-align: top;">
                                        <b>{{ $key }}</b> <br><br>
                                        @foreach ($success_fail_count[$key] as $stat => $cnt)
                                            @if ($stat == 1)
                                                <span class="text-green">Successful : {{ $cnt }} ({{ ($total_succ_dec_cnt > 0) ? round(($cnt/$total_succ_dec_cnt)*100, 2) : 0 }}%)</span> <br>
                                            @elseif($stat == 0)
                                                <span class="text-red">Declined : {{ $cnt }} ({{ ($total_succ_dec_cnt > 0) ? round(($cnt/$total_succ_dec_cnt)*100, 2) : 0 }}%)</span> <br>
                                            @elseif($stat == 5)
                                                <span class="text-primary">Blocked : {{ $cnt }}</span> <br>
                                            @endif
                                        @endforeach
                                    </td>
                                    @endif
                                    @foreach ($value as $ky => $val)
                                        <td>{!! ($val->reason == "") ? "<span class='text-red'> -- N/A -- </span?" : $val->reason !!}</td>
                                        <td class="text-center">{{ $val->transaction_count }} </td>
                                        </tr>
                                    @endforeach
                                    <?php $i++; ?>
                                @endforeach
                                ...
                                @if (count($data) > 0)
                                    @foreach ($data as $key => $value)
                                        ...
                                        <tr>
                                            ...
                                            @foreach ($value as $ky => $val)
                                                <?php
                                                $ttl_succ_dec_cnt = ($bank_name_txn_cnt[$key][$ky][0] ?? 0) + ($bank_name_txn_cnt[$key][$ky][1] ?? 0);
                                                $j = 0;
                                                ?>

                                                @if ($j == 0)
                                                    <td rowspan="{{ $val->count() }}" style="vertical-align: top;">
                                                        <b>{{ $ky }}</b> <br>
                                                        @foreach ($bank_name_txn_cnt[$key][$ky] as $stt => $cunt)
                                                            @if ($stt == 1)
                                                                <span class="text-green">Successful : {{ $cunt }}
                                                                    ({{ $ttl_succ_dec_cnt > 0 ? round(($cunt / $ttl_succ_dec_cnt) * 100, 2) : 0 }}%)
                                                                </span>
                                                                <br>
                                                            @elseif($stt == 0)
                                                                <span class="text-red">Declined : {{ $cunt }}
                                                                    ({{ $ttl_succ_dec_cnt > 0 ? round(($cunt / $ttl_succ_dec_cnt) * 100, 2) : 0 }}%)</span>
                                                                <br>
                                                            @elseif($stt == 5)
                                                                <span class="text-primary">Blocked :
                                                                    {{ $cunt }}</span> <br>
                                                ...
                                                @foreach ($val as $k => $vl)
                                                    <td>{{ $vl->reason }}</td>
                                                    <td class="text-center">{{ $vl->transaction_count }} </td>
                                        </tr>
                                    @endforeach
