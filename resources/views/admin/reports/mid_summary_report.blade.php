...
@section('content')
    ...
    <div class="row">
        <div class="col-md-6">
            <h4 class="me-50">MID Summary Report</h4>
        </div>
        <div class="col-md-6 text-right">
            <div class="btn-group btn-shadow">
                ...
                <a href="{{ route('mid-summary-report') }}" class="btn btn-danger btn-sm"
                    style="border-radius: 0px 5px 5px 0px !important;">Reset</a>
            </div>
            @if (auth()->guard('admin')->user()->can(['export-mid-summary-report']))
                @if (count($transactions_summary) > 0)
                    <a class="btn btn-primary btn-sm "
                        data-link="{{ route('mid-summary-report-excle', request()->all()) }}"
                        data-filename="Mid_Summary_Report_Excel_" href="#" id="ExcelLink">
                                                ...
                                                @if (count($transactions_summary) > 0)
                                                    @foreach ($transactions_summary as $mid_transaction)
                                                        ...
                                                        foreach ($mid_transaction as $transaction) {
                                                            $usd_amount += $transaction['amount_in_usd'];
                                                        }
                                                        ?>
                                                        @foreach ($mid_transaction as $transaction)
                                                            <tr>
                                                                @if ($k == 0)
                                                                    <td>{{ $transaction['bank_name'] }}<br>({{ '$ ' . $usd_amount }})
                                                                    </td>
                                                                @else
                                                                    <td></td>
                                                                @endif
                                                    ...
                                                    @foreach ($transactions_summary as $mid_transaction)
                                                        ...
                                                        @foreach ($mid_transaction as $transaction)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    {{ round($transaction['retrieval_percentage'], 2) }}
                                                                </td>
                                                            </tr>
                                                            <?php $k++; ?>
                                                        @endforeach
