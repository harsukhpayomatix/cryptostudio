...
@section('breadcrumbTitle')
    <a href="{{ route('admin.dashboard') }}">Dashboard</a> / Removed Suspicious Transactions
@endsection
    ...
    <div class="row">
        <div class="col-lg-12 text-right mb-2">
            @if (auth()->guard('admin')->user()->can(['export-all-transaction']))
                <a href="javascript:;" data-link="{{ route('merchant-remove-flagged.export', request()->all()) }}"
                    data-filename="Remove_Flagged_Transaction_Excel_" class="btn btn-primary" id="ExcelLink"><i
            ...
            <div class="card">
                <div class="card-header">
                    <div class="iq-header-title">
                        <h4 class="card-title">Removed Suspicious Transactions</h4>
                    </div>
                    <div>
                        ...
                        <div class="btn-group">
                            ...
                            <a href="{{ route('merchant-remove-flagged') }}" class="btn btn-danger btn-sm"
                                ...
                                @if (count($data) > 0)
                                    @foreach ($data as $transaction)
                                        <tr id="tr_{{ $transaction->id }}">
                                            ...
                                            <td>
                                                {{ \Carbon\Carbon::parse($transaction->falgged_date)->format('d-m-Y / H:i:s') }}
                                            </td>
                                            <td>
                                                {{ convertDateToLocal($transaction->flagged_remove_date, 'd-m-Y / H:i:s') }}
                                            </td>
                                            ...
                                            <td>
                                                <div class="form-check">
                                                    <input type="checkbox" name="refund" class="form-check-input"
                                                        id="refund{{ $transaction->id }}"
                                                        data-id="{{ $transaction->id }}" checked disabled=""
                                                        data-bs-toggle="modal" href="#transactionRefund">
                                                    <label for="refund{{ $transaction->id }}" class="form-check-label">
                                                    </label>
                                                </div>
                                            </td>
                                            @if (auth()->guard('admin')->user()->can(['details-transaction']))
