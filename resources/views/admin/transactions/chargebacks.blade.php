                    ...
                    <div class="modal-body">
                        <div class="basic-form">
                            <div class="row ">
                                ...
                                <div class="form-group col-lg-6">
                                    <label>Chargeback Start Date</label>
                                    <div class="date-input">
                                ...
                                <div class="form-group col-lg-6">
                                    <label>Chargeback End Date</label>
                                    <div class="date-input">
                                ...
                                <div class="form-group col-lg-6">
                                    ...
                                    <select class="form-control input-rounded select2"
                                        name="transactions_document_upload_files" id="document_status">
                                        <option selected disabled> -- Select Status -- </option>
                                        <option value="0"
                                            {{ isset($_GET['transactions_document_upload_files']) && $_GET['transactions_document_upload_files'] == '0' ? 'selected' : '' }}>
                                            Pending</option>
                                        <option value="1"
                                            {{ isset($_GET['transactions_document_upload_files']) && $_GET['transactions_document_upload_files'] == '1' ? 'selected' : '' }}>
                                            Uploaded</option>
                                    </select>
                                </div>
            ...
            @if (auth()->guard('admin')->user()->can(['export-all-transaction']))
                @if (!empty($subQueryString))
                    <a href="{{ route('all-admin-chargeback-transactions-csv-export', [$subQueryString]) }}"
                        class="btn btn-primary btn-sm"><i class="fa fa-download"></i> Export Excel </a>
                @else
                ...
                <div class="card-header">
                    ...
                    <div>
                        ...
                        <div class="btn-group">
                            ...
                            <a href="{{ route('admin.chargebacks') }}" class="btn btn-danger btn-sm"
...
@section('customScript')
    <script type="text/javascript">
        ...
        $('body').on('click', '#resendChargebacksEMail', function() {
            var id = [];
            $('.multiselect:checked').each(function() {
                id.push($(this).val());
            });
            if (id.length > 0) {
                $.ajax({
                    url: '{{ route('resend-chargebacks-email') }}',
                    method: "POST",
                    context: $(this),
                    data: {
                        '_token': CSRF_TOKEN,
                    ...
                    success: function(data) {
                        if (data.success) {
                            ...
                            $(this).attr('disabled', false);
                            $(this).html(' Send Chargebacks Email');
                            window.setTimeout(
                                function() {
                                    location.reload(true)
                                },
