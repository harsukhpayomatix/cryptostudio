...
@section('title')
    Retrival Transactions
@endsection

@section('breadcrumbTitle')
    <a href="{{ route('admin.dashboard') }}">Dashboard</a> / Retrival Transactions
@endsection
@section('content')
    @include('requestDate')
        ...
        <div class="col-lg-12 text-right mb-2">
            ...
            @if (auth()->guard('admin')->user()->can(['export-all-transaction']))
                @if (!empty($subQueryString))
                    <a href="{{ route('all-admin-retrival-transactions-csv-export', [$subQueryString]) }}"
                        class="btn btn-primary"><i class="fa fa-download me-2"></i> Export Excel </a>
                @else
                    <a href="{{ route('all-admin-retrival-transactions-csv-export') }}" class="btn btn-primary"
                        id="ExcelLink">
            ...
            @if (auth()->guard('admin')->user()->can(['delete-all-transaction']))
                <button type="button" class="btn btn-primary" id="deleteSelected"
                    ...
                    Record</button>
            @endif
            @if (auth()->guard('admin')->user()->can(['send-mail-retrival-transaction']))
                <button type="button" class="btn btn-primary" id="resendRetrievalEmail"> Send
                    Retrieval Email</button>
            @endif
        </div>
    </div>
    <div class="row">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="header-title">
                        <h4 class="card-title">Retrival Transaction</h4>
                    </div>
                    <div>
                        ...
                        <div class="btn-group">
                            ...
                            <a href="{{ route('admin.retrieval') }}" class="btn btn-danger btn-sm"
                        ...
                        <table class="table table-borderless table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th>Order No</th>
                                    <th style="min-width: 210px;">Retrival Date & Time</th>
                                    ...
                                    @if (auth()->guard('admin')->user()->can(['update-retrival-transaction']))
                                        <th>Retrival</th>
                                    @endif
                                        ...
                                        <tr id="tr_{{ $transaction->id }}">
                                            ...
                                            <td>
                                                ...
                                                @elseif($transaction->card_type == '1')
                                                    <label class="badge-sm badge badge-primary">Amex</label>
                                                @elseif($transaction->card_type == '4')
                                                    <label class="badge-sm badge badge-primary">Discover</label>
                                                @endif
                                            ...
                                            <td>
                                                {{ \Carbon\Carbon::parse($transaction->retrieval_date)->format('d-m-Y / H:i:s') }}
                                            </td>
                                            <td>
                                                <span>
                                                ...
                                                <td>
                                                    <div class="form-check">
                                                        <input type="checkbox" data-id="{{ $transaction->id }}"
                                                            class="form-check-input retrivalCheckBox"
                                                            id="customCheckBox{{ $transaction->id }}"
                                                            @if ($transaction->is_retrieval) checked @else disabled @endif>
                                                        <label class="form-check-label"
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        @if (auth()->guard('admin')->user()->can(['update-retrival-transaction']))
                                                            <a data-bs-toggle="modal"
                                                                ...
                                                                class="retrieval-show-document dropdown-item"
                                                                data-id="{{ $transaction->id }}"
                                                                data-link="{{ route('merchant-retrieval-show-documents') }}">Show
                                                                Document</a>
                                                        @endif
                                                    </div>
                                                </div>
...
@section('customScript')
    ...
    <script>
        ...
        $(document).on('click', '.retrivalCheckBox', function() {
            var id = $(this).data('id')
            var check = !$(this).prop('checked')
            if (check) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.remove.retrieval') }}",
                    data: {
                        id: id,
                        _token: "{{ csrf_token() }}",
                    },
        ...
        $('body').on('click', '#resendRetrievalEmail', function() {
            var id = [];
            ...
            if (id.length > 0) {
                $.ajax({
                    url: '{{ route('resend-retrieval-email') }}',
                    method: "POST",
                    context: $(this),
                    data: {
                        '_token': CSRF_TOKEN,
                    ...
                    success: function(data) {
                        if (data.success) {
                            ...
                            $(this).html(' Send Retrieval Email');
                            window.setTimeout(
                                function() {
