...
@section('content')
    ...
    {{ Form::model($article, array('route' => array('article.update', $article->id), 'method' => 'PUT', 'class' => 'form form-horizontal', 'enctype'=>'multipart/form-data','id'=>'article-form')) }}
    <div class="common-section mb-5">
        <div class="col-xl-12 col-sm-12 col-md-12 col-12">
            <div class="heading-title mb-4">
                <h3 class="mb-0"> Article Edit Form </h3>
                            ...
                            <select name="tag_id[]" multiple="multiple" class="select2 btn-primary fill_selectbtn_in own_selectbox" data-width="100%" id="tag_id">
                                ...
                                @if($tags)
                                    @foreach($tags as $value)
                                        <option value="{{ $value->id }}" {{in_array($value->id, $article->tag_id) ? 'selected' : ''}}>{{ $value->title }}</option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="row mx-auto">
                                ...
                                <div class="col-2 col-md-2">
                                    <a href="{{ Config('app.aws_path').$article->image }}" target="_blank" class="btn btn-yellow">Show</a>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
