...
@section('title')
    Generated Payout Reports for WL RP
@endsection

@section('breadcrumbTitle')
    <a href="{{ route('admin.dashboard') }}">Dashboard</a> / Generated Payout Reports for WL RP
@endsection
...
@section('content')
                                    ...
                                    <select name="user_id" data-size="7" data-live-search="true"
                                        ...
                                        @foreach ($data as $company)
                                            <option value="{{ $company->id }}"
                                                {{ isset($_GET['user_id']) && $_GET['user_id'] == $company->id ? 'selected' : '' }}>
                                                {{ $company->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    @if (auth()->guard('admin')->user()->can(['form-generate-wl-rp-payout-reports']))
        <div class="row">
            <div class="col-xl-12 col-xxl-12">
                <div class="card  mt-1">
                    <div class="card-header">
                        <div class="iq-header-title">
                            <h4 class="card-title">Generate Report for WL RP</h4>
                        </div>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('payout-report-store-rp') }}" method="post" id="search-form"
                            class="form-dark">
                            @csrf
                            <div class="row">
                                ...
                                <div class="col-xl-3 col-sm-3 col-12 col-md-6 mt-3">
                                    <div class="form-group">
                                        <label>Show RP Side</label>
                                        <div class="form-check">
                                            <input type="checkbox" id="show_client_side check1" name="show_client_side"
                                                value="1" class="form-check-input">
                                            <label class="form-check-label" for="check1">Show on RP Side</label>
                                        </div>
                                    </div>
                                ...
                                <div class="form-group col-lg-12 mt-3">
                                    ...
                                    <a href="{{ route('generate-payout-report-rp') }}"
    ...
    <div class="row">
        <div class="col-xl-12 col-xxl-12">
            <div class="card  mt-1">
                <div class="card-header">
                    <div class="iq-header-title">
                        <h4 class="card-title">Generated Payout Reports for WL RP</h4>
                    </div>
                    <div>
                        @php
                            $url = Request::fullUrl();
                        ...
                        @endphp
                        @if (auth()->guard('admin')->user()->can(['export-generated-wl-rp-payout-reports']))
                            @if (!empty($subQueryString))
                                <a href="{{ route('admin.generate_report.rp.export', [$subQueryString]) }}"
                                    class="btn btn-primary btn-sm" data-filename="GenerateReport_Excel_"><i
                                        class="fa fa-download"></i>
                                    Download Excel</a>
                            @else
                                <a href="{{ route('admin.generate_report.rp.export') }}" class="btn btn-primary btn-sm"
                                    data-filename="GenerateReport_Excel_"><i class="fa fa-download"></i>
                                    Download Excel</a>
                            @endif
                        @endif
                        @if (auth()->guard('admin')->user()->can(['delete-generated-wl-rp-payout-reports']))
                            <a id="bulk_delete" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i> Delete Selected
                                Reports </a>
                        @endif
                        ...
                        <div class="btn-group">
                            ...
                            <a href="{{ route('generate-payout-report-rp') }}" class="btn btn-danger btn-sm">Reset</a>
                        </div>
                    </div>
                </div>
                @php
                        ...
                        <table id="payout_Report" class="table table-striped table-borderless">
                            <thead>
                                <tr>
                                    ...
                                    @if (auth()->guard('admin')->user()->can(['update-generated-wl-rp-payout-reports']))
                                        <th class="text-center">Paid</th>
                                        <th class="text-center" style="min-width: 170px;">Show Client Side </th>
                                    @endif
                                    @if (auth()->guard('admin')->user()->can(['show-generated-wl-rp-payout-reports']))
                                    ...
                                    @foreach ($dataT as $key => $value)
                                        <tr>
                                            ...
                                            @if (auth()->guard('admin')->user()->can(['update-generated-wl-rp-payout-reports']))
                                                ...
                                                <td class="text-center">
                                                    <div class="custom-control custom-checkbox form-check mr-0 pull-left">
                                                        <input type="checkbox" name=""
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        @if (auth()->guard('admin')->user()->can(['show-generated-wl-rp-payout-reports']))
                                                            <a class="dropdown-item"
                                                                href="{{ route('generate_report.pdf.rp', $value->id) }}"><i
                                                                    class="fa fa-download text-secondary me-2"></i>PDF</a>


                                                            <a href="{{ route('generate_report.show.rp', $value->id) }}"
                ...
                <div class="modal-body" id="detailsContent">
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <form action="{{ route('generate-report-document-rp') }}" method="POST"
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', '.paidstatus', function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    context: $(this),
                    url: "{{ URL::route('make-report-paid-rp') }}",
                    data: {
                        '_token': CSRF_TOKEN,
                        'status': status,
                        'id': id
            ...
            $('body').on('click', '.showClientSide', function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    context: $(this),
                    url: "{{ URL::route('show-report-client-rp') }}",
                    data: {
                        '_token': CSRF_TOKEN,
                        'status': status,
                        'id': id
