...
Route::group(['middleware' => ['api', 'cors']], function () {
	...
	// test api
	Route::post('test/transaction', 'API\TestDirectApiController@store')->name('test.transaction');

	// api v2
	Route::post('v2/transaction', 'API\ApiController@store')->name('v2.transaction');
	...
	Route::post('get/transaction', 'API\DirectApiController@getTransaction')->name('get-transaction-details');
	...
	// NowPayment gateway
	Route::post('nowpayments-crypto-callback/{session}/{ipn}', 'Repo\PaymentGateway\NowPayments@callback')->name('nowpayments-crypto-callback');
