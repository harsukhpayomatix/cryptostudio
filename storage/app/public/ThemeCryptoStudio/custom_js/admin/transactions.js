    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.success == true || data.success == false) {
                setTimeout(function () {
                    $("#transactionRefund").modal("hide");
                    $("#transactionShowModal").modal("hide");
                }, 1000);
            }
        },
        error: function (res) {
...
// * Cancel the refund modal
$(document).on("click", ".cancelRefundBtn", function () {
    $(".clRefund").prop("checked", false);
});
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.success == true || data.success == false) {
                setTimeout(function () {
                    // location.reload();
                    $("#transactionChargebacks").modal("hide");
                    $("#transactionShowModal").modal("hide");
                }, 1000);
            }
        },
...
// * On cancel button remove the checked mark
$(document).on("click", ".cancelChargeback", function () {
    $(".clChargeback").prop("checked", false);
});

$(document).on("click", ".clRetrieval", function () {
    var id = $(this).data("id");
...
$("body").on("click", "#submitRetrieval", function (e) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.success == true || data.success == false) {
                setTimeout(function () {
                    $("#transactionRetrieval").modal("hide");
                    $("#transactionShowModal").modal("hide");
                }, 1000);
            }
        },
...
// Unchecked the retrieval input on cancel
$(document).on("click", ".closeRetrivalsBtn", function () {
    $(".clRetrieval").prop("checked", false);
});
...
// Cancel the flagged
$(document).on("click", ".cancelFlaggedBtn", function () {
    $(".clFlagged").prop("checked", false);
});
