...
class Transaction extends Model
{
    ...
    public function filterTransactionData($input, $data)
    {
        ...
        if ((isset($input['flagged_start_date']) && $input['flagged_start_date'] != '') && (isset($input['flagged_end_date']) && $input['flagged_end_date'] != '')) {
            $start_date = date('Y-m-d', strtotime($input['flagged_start_date']));
            $end_date = date('Y-m-d', strtotime($input['flagged_end_date']));
    ...
    public function storeData($input)
    {
        ...
        $user = User::select('name', 'email', 'merchant_transaction_notification')
            ->where('id', $input['user_id'])
            ->first();
        // send transaction notification
        if (!empty($user) && $input['status'] == '1' && $user->merchant_transaction_notification == '1') {
            try {
                $mailInput = $input;
                $mailInput['user_name'] = $user->name;
                \Mail::to($user->email)->queue(new TransactionMail($mailInput));
            } catch (\Exception $e) {
                \Log::info(['transaction_mail_send' => $e->getMessage()]);
            }
        }
        return $transaction;
    }
...
SQL;
    ...
    public function getAllMerchantTransactionData($input, $noList)
    {
        ...
        $data = static::select("order_id", "id", "email", "amount", "is_request_from_vt", "payment_type", "card_type", "is_white_label", "request_origin", "request_from_ip", "payment_gateway_id", "status", "amount", "created_at", "currency", "is_converted", "converted_currency", "converted_amount", "country", "user_id")
