...
class AdminAllPreArbitrationTransactionsCSVExport
{
    ...
    public function download(Request $request)
    {
        ...
        return response()->streamDownload(function() use($columns, $input) {
            ...
            $data = Transaction::select('transactions.order_id','applications.business_name','transactions.first_name',
                    ...
                    'transactions.reason',
                    \DB::raw('(CASE
                        WHEN transactions.is_pre_arbitration = "1" THEN "Yes"
                        ELSE "No"
                        END) AS is_pre_arbitration'),
                    \DB::raw("(DATE_FORMAT(transactions.pre_arbitration_date, '%d-%m-%Y %h:%i:%s')) AS pre_arbitration_date"),
                    \DB::raw('(CASE
            ...
            if ((isset($input['prearbitration_start_date']) && $input['prearbitration_start_date'] != '') && (isset($input['prearbitration_end_date']) && $input['prearbitration_end_date'] != '')) {
                $start_date = $input['prearbitration_start_date'];
                $end_date = $input['prearbitration_end_date'];
            ...
            fclose($file);
        }, 'Pre_Arbitration_Transaction_Excel_'.date('d-m-Y').'.csv');
    }
}
