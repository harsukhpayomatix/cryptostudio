...
class MerchantTestTransactionmanagementCSVExport
{
    protected $id;
    ...
    public function download(Request $request)
    {
        ...
        $data = Transaction::select('transactions.order_id','transactions.customer_order_id','applications.business_name','transactions.first_name',
            ...
            $data = $data->whereIn('transactions.payment_gateway_id', ['1', '2'])->orderBy('transactions.id', 'DESC');
            $data = $data->cursor()
            ...
            fclose($file);
        }, 'Test_Transaction_Excel_'.date('d-m-Y').'.csv');     
    }
}
