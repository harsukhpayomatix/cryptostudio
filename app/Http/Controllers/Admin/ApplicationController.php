...
class ApplicationController extends AdminController
{
    ...
    public function approved_application(Request $request)
    {
        ...
        if ($agent) {
            ...
            if ($agent->add_buy_rate_master && $agent->add_buy_rate_master > $input['merchant_discount_rate_master_card']) {
                $error['merchant_discount_rate_master_error'] = 'Merchant discount rate for master must be greater than ' . $agent->add_buy_rate_master . '.';
            }
            if ($agent->add_buy_rate_amex && $agent->add_buy_rate_amex > $input['merchant_discount_rate_amex_card']) {
                $error['merchant_discount_rate_amex_error'] = 'Merchant discount rate for amex must be greater than ' . $agent->add_buy_rate_amex . '.';
            }
            if ($agent->add_buy_rate_discover && $agent->add_buy_rate_discover > $input['merchant_discount_rate_discover_card']) {
                $error['merchant_discount_rate_discover_error'] = 'Merchant discount rate for discover must be greater than ' . $agent->add_buy_rate_discover . '.';
            }
        }
        ...
        if ($this->Application->updateApplication($request->get('id'), ['status' => '4', 'reason_reassign' => ''])) {
            ...
            $updateUser = User::where('id', $application->user_id)
                ->update([
                    ...
                    'merchant_discount_rate_master_card' => $request->get('merchant_discount_rate_master_card'),
                    'merchant_discount_rate_amex_card' => $request->get('merchant_discount_rate_amex_card'),
                    'merchant_discount_rate_discover_card' => $request->get('merchant_discount_rate_discover_card'),
                    'merchant_discount_rate_upi' => $request->get('merchant_discount_rate_upi'),
                    'merchant_discount_rate_crypto' => $request->get('merchant_discount_rate_crypto'),
                    'rolling_reserve_paercentage' => $request->get('rolling_reserve_paercentage'),
                    'setup_fee' => $request->get('setup_fee'),
                    'setup_fee_master_card' => $request->get('setup_fee_master_card'),
                    'setup_fee_amex_card' => $request->get('setup_fee_amex_card'),
                    'setup_fee_discover_card' => $request->get('setup_fee_discover_card'),
                    ...
                    "agent_commission_master_card" => $request->get("agent_commission_master"),
                    'is_rate_sent' => '1'
                ]);
            // dd($request->all(), $input, $request->get("agent_commission_master"), $request->get("agent_commission_master"), $updateUser);
            try {
            ...
            $notification = [
                ...
                'body' => 'Congratulations! Your application has just been approved.',
                'url' => '/my-application',
                'is_read' => '0'
            ];
    ...
    public function applicationAgreementSent(Request $request)
    {
        ...
        try {
            ...
            $data['url'] = URL::to('/') . '/agreement-documents-upload?userId=' . $user->id . '&token=' . $token;
