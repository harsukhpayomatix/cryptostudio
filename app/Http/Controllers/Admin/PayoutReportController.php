...
class PayoutReportController extends AdminController
{
    ...
    public function autoReportingChild($value, $input, $userData, $user_apm_ids, $current_payout_report, $old_payout_report, $type, $rates)
    {
        ...
        if ($type == 'Crypto') {
            $common_transaction = $common_transaction->where('payment_type', 'crypto');
        } elseif ($type == 'UPI') {
            $common_transaction = $common_transaction->where('payment_type', 'upi');
        } elseif ($type == 'Visa') {
            $common_transaction = $common_transaction->where('payment_type', 'card')
                ->whereNotIn('card_type', ['1', '3', '4']);
        } elseif ($type == 'MasterCard') {
            $common_transaction = $common_transaction->where('payment_type', 'card')
                ->where('card_type', '3');
        ...
        if ($type == 'Crypto') {
            $childData['mdr'] = ($userData->merchant_discount_rate_crypto * $approved_transaction->amount) / 100;
        } elseif ($type == 'UPI') {
            $childData['mdr'] = ($userData->merchant_discount_rate_upi * $approved_transaction->amount) / 100;
        } elseif ($type == 'Visa') {
            ...
            $childData['mdr'] = ($userData->merchant_discount_rate_amex_card * $approved_transaction->amount) / 100;
        } elseif ($type == 'Discover') {
            $childData['mdr'] = ($userData->merchant_discount_rate_discover_card * $approved_transaction->amount) / 100;
        }
        ...
        if ($type == "Crypto") {
            $past_flagged_sum_deduction = (($userData->merchant_discount_rate_crypto * $total_past_flagged->amount) / 100) + (($userData->rolling_reserve_paercentage * $total_past_flagged->amount) / 100);
        } elseif ($type == "UPI") {
            $past_flagged_sum_deduction = (($userData->merchant_discount_rate_upi * $total_past_flagged->amount) / 100) + (($userData->rolling_reserve_paercentage * $total_past_flagged->amount) / 100);
        } elseif ($type == "Visa") {
            ...
            $past_flagged_sum_deduction = (($userData->merchant_discount_rate_amex_card * $total_past_flagged->amount) / 100) + (($userData->rolling_reserve_paercentage * $total_past_flagged->amount) / 100);
        } elseif ($type == "Discover") {
            $past_flagged_sum_deduction = (($userData->merchant_discount_rate_discover_card * $total_past_flagged->amount) / 100) + (($userData->rolling_reserve_paercentage * $total_past_flagged->amount) / 100);
        }
        ...
        if ($type == "Crypto") {
            $past_retrieval_sum_deduction = (($userData->merchant_discount_rate_crypto * $total_past_retrieval->amount) / 100) + (($userData->rolling_reserve_paercentage * $total_past_retrieval->amount) / 100);
        } elseif ($type == "UPI") {
            $past_retrieval_sum_deduction = (($userData->merchant_discount_rate_upi * $total_past_retrieval->amount) / 100) + (($userData->rolling_reserve_paercentage * $total_past_retrieval->amount) / 100);
        } elseif ($type == "Visa") {
            ...
            $past_retrieval_sum_deduction = (($userData->merchant_discount_rate_amex_card * $total_past_retrieval->amount) / 100) + (($userData->rolling_reserve_paercentage * $total_past_retrieval->amount) / 100);
        } elseif ($type == "Discover") {
            $past_retrieval_sum_deduction = (($userData->merchant_discount_rate_discover_card * $total_past_retrieval->amount) / 100) + (($userData->rolling_reserve_paercentage * $total_past_retrieval->amount) / 100);
        }
