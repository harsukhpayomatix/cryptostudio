...
class Itexpay extends Controller
{
    ...
    public function checkout($input, $check_assign_mid)
    {
        ...
        curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($token_data));
        $token_json = curl_exec($curl);
        curl_close($curl);
    ...
    public function submit(Request $request, $session_id, $encrypt)
    {
        ...
        $token_response = json_decode($transaction_session->response_data, 1);
        ...
        if (
            ...
            if (
                ...
                } elseif (isset($response['code']) && $response['code'] == 'error') {
                    $input['status'] = '0';
    ...
    private function encryptData($transactionData, $encryptedPublicKey)
    {
        try {
            ...
            $secKey = openssl_random_pseudo_bytes(16); // 128 bits = 16 bytes
            $SencryptKey = base64_encode($secKey);
            $iv = openssl_random_pseudo_bytes(16); // 128 bits = 16 bytes IV

            // Encrypt the AES secret key with RSA
            // Encrypt the AES secret key with RSA
            $publicKey = openssl_pkey_get_public("-----BEGIN PUBLIC KEY-----
" .
            chunk_split($encryptedPublicKey, 64, "
") .
            "-----END PUBLIC KEY-----");

            // Define the variable to store the encrypted key
            $encryptedKey = '';

            openssl_public_encrypt($SencryptKey, $encryptedKey, $publicKey);
            $encyptedkey = base64_encode($encryptedKey);

            $encrypteddata = openssl_encrypt($transactionData, 'aes-128-cbc', $secKey, OPENSSL_RAW_DATA,$iv);
            $encrypteddata = base64_encode($encrypteddata);

            return ["ctx"=>  $encyptedkey, "data"=>$encrypteddata];
        } catch (\Exception $e) {
    ...
    private function getMessages()
    {
        $messages = [
            ...
            'Q1' => 'account authentication failed',
            'Q1' => 'token authentication failed',
            '01' => 'Refer to card issuer',
            '65' => 'authentication required',
            '65' => 'payment not allowed',
            ...
            '12' => 'duplicate transaction reference',
        ];
