...
class Payaza extends Controller
{
    ...
    public function checkout($input, $midDetails)
    {
        ...
        // Log::info(["payaza-request-payload" => $payload, "payaza-response" => $response]);
        // * Store mid payload 
    ...
    public function payazaReturn(Request $request, $id)
    {
        ...
        if ($payload["statusOk"] == true && $payload["message"] == "Approved") {
            $input["status"] = "1";
            $input["reason"] = isset($payload["debugMessage"]) ? $payload["debugMessage"] : "Your transaction processed successfully!";
        } else if ($payload["statusOk"] == false && $payload["status"] == "Failed") {
            $input["status"] = "0";
            $input["reason"] = isset($payload["debugMessage"]) ? $payload["debugMessage"] : "Your transaction could not processed!";
