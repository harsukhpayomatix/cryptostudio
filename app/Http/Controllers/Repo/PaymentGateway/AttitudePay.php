...
class AttitudePay extends Controller
{
    ...
    public function checkout($input, $check_assign_mid)
    {
        ...
        try {
            $payload = [
                ...
                "state" => $input['country'],
                "redirect_url" => route('attitudepay.redirect', [$input['session_id']]),
                "server_callback_url" => route('attitudepay.callback', [$input['session_id']]),
                // "server_callback_url" => "https://webhook.site/9852e50b-88eb-4e86-8b33-b77be7419e23",
            ...
            $statusShaString = self::LOGIN . $input['order_id'] . $input['gateway_id'] . $check_assign_mid->control;
            $statusPayload = [
                'login' => self::LOGIN,
            ...
            // Log::info(["altitude-status-api-response" => $statusResponse]);
    ...
    public function redirect(Request $request, $sessionId)
    {
        ...
        // Log::info([
        //     'attitudepay-request-redirect' => json_encode($response),
        // ]);
    ...
    public function webhook(Request $request, $sessionId)
    {
        ...
        $transactionSession = DB::table('transaction_session')
            ->select("request_data")->where('transaction_id', $sessionId)->first();
