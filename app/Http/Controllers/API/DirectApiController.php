...
class DirectApiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($input, [
            'first_name' => 'nullable|min:3|max:100|regex:/^[a-zA-Z\s]+$/',
            'last_name' => 'nullable|min:2|max:100|regex:/^[a-zA-Z\s]+$/',
    ...
    {
        // only accept parameters that are available
        $input = $request->only(['customer_order_id', 'order_id']);
        $api_key = $request->bearerToken();
        ...
        if ($request->order_id == null && $request->customer_order_id == null) {
            $input['status'] = 6;
            $input['reason'] = 'order_id or customer_order_id field is required.';
            return ApiResponse::notFound($input);
        }

        $transaction = $transaction = Transaction::where('user_id', $user->id);
        ...
        if ((isset($input['customer_order_id']) && $input['customer_order_id'] != null)) {
            $transaction = $transaction->where('customer_order_id', $input['customer_order_id']);
        }
        ...
        if (empty($transaction)) {
            ...
            return ApiResponse::status($transaction);
        }
    }
        ...
        if ($transaction != null) {
            ...
            if ($transaction->refund == '0' && $transaction->chargebacks == '0' && $transaction->is_flagged == '0' && $transaction->is_retrieval == '0' && $transaction->status == '1') {
                ...
                try {
                    ...
                    // * Admin Notification 
                    Mail::to("tushar.vashishth@pay.agency")->cc(["kinnari.trivedi@paypound.ltd"])->queue(new AdminRefundNotification($transaction));
