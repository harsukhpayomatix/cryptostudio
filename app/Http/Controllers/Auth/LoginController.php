...
class LoginController extends Controller
{
    ...
    */
    ...
    public function login(Request $request)
    {
        ...
        if (!$userData) {
            \Session::put('error', "We couldn't find an account associated with this email.");
            return redirect()->back();
        }

        if ($userData['is_white_label'] == '1') {
            if ($userData['is_login_wl_merchant'] == '0') {
                \Session::put('error', 'You are not able to login.');
                return redirect()->back();
            }
        }
        ...
        if ($userData['is_active'] == '1') {
            ...
            return back()->with('error', 'Your account has not been activated till now.');
        }
    }
